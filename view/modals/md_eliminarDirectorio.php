<form id="inhabilitarDirectorio">
<div class="modal " id="eliminarDirectorio" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">

      <div class="modal-body">
      <input type="hidden" id="mat" name="idMaterial">
      <input type="hidden" name="funcion" value="inhabilitar"> 
      <input type="hidden" id="pro" name="idProveedor">
      <h2 class="text-center text-muted">¿Esta seguro?</h2>
	  <p class="lead text-muted text-center" style="display: block;margin:10px">Esta acción eliminará de forma permanente el registro. ¿Deseas continuar?</p>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-lg btn-default" data-dismiss="modal">Cancelar</button>
        <button type="submit" class="btn btn-lg btn-primary">Continuar</button>
      </div>
    </div>
  </div>
</div>
</form>